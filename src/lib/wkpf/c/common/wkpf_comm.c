...
void wkpf_comm_handle_message(void *data) {
	...
	switch (msg->command) {
      ...
      uint8_t until_index = 0;
        ...
        payload[4*i + 3] = (uint8_t)(wuclass->wuclass_id >> 8);
        payload[4*i + 4] = (uint8_t)(wuclass->wuclass_id);
				if (wuclass->flags & WKPF_WUCLASS_FLAG_APP_CAN_CREATE_INSTANCE) {
					payload[4*i + 5] = WKPF_IS_VIRTUAL_WUCLASS(wuclass) ? 3 : 2;
				} else {
					payload[4*i + 5] = WKPF_IS_VIRTUAL_WUCLASS(wuclass) ? 1 : 0;
        }
			}
